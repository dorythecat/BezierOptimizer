...
def quad_bezier(p0: float, p1: float, p2: float) -> str:
    ...
    if p0 == p1:
        output = ("" if p1 == 0 else str(p1)) + ("" if p2[3:-3] == "0" else (p2 + "t²"))
    ...
    if p0[3:] == p2[3:]: # Ignore signs
        if p1 == "":
            return ""
        return ("" if p1 == "1" else ("-" if p1 == "-1" else p1)) + p0[:-3] + "((1 - t)² + t²)"
    p0 = ((("- " if p0 == " - " else "") + "1") if p0 in [" + ", " - "] and p1 == "" else (p0 + "(1 - t)²"))
    return p1 + p0 + ("" if p2[3:-3] == "0" else (p2 + "t²"))

def cubic_bezier(p0: float, p1: float, p2: float, p3: float) -> str:
    b1 = quad_bezier(p0, p1, p2) # Precalculate curve
    b1 = ("" if b1 == "" else ("(1 - t) * " + "(" + b1 + ")")) # Turn to string
    # Same as above
